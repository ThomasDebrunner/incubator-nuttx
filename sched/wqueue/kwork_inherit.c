...
#if CONFIG_SEM_NNESTPRIO > 0
      {
         /* Priority is supposed to go back to what it was before. We can remove the
          * highest boost for kwork boosts from our list of boosts, and
          * re-evaluate what is the now the highest priority still waiting */
          ...
          if (max_boost_index >= 0) {
            ...
            * Remove this, as this is no longer required */
            ...
            wtcb->sem_boosts[max_boost_index] = wtcb->sem_boosts[wtcb->nsem_boosts-1];
            wtcb->nsem_boosts--;
          }
      }
